...
from torchvision import transforms
from .base_dataset import BaseDataset, NoriBaseDataset, Local_norm
...
class InpaintDataset(BaseDataset):
    ...
    def __init__(self, img_flist_path, mask_flist_paths_dict,
                resize_shape=(256, 256), transforms_oprs=['resize','to_tensor'],
    ...
    def read_img(self, path):
        ...
        # img = Image.fromarray(np.uint8(255*img_nrm)).convert('RGB')
    ...
    def random_ff_mask(config):
        ...
        mask = np.zeros((h,w))
        num_v = 2+np.random.randint(config['mv'])#tf.random_uniform([], minval=0, maxval=config.MAXVERTEX, dtype=tf.int32)

        for i in range(num_v):
            start_x = np.random.randint(int(0.3*w),int(0.7*w))
            start_y = np.random.randint(int(0.3*h),int(0.7*w))
